...
Assign a value to object 'x'. The '<-' symbol is read as 'gets'. In this case, x gets 42. Make sure that you are in the environment panel and you should see the value associated with 'x'. On a mac, you can push `option` and `-` to automatically generate the gets symbol.  The object x gets the value 42
```{r}
x <- 42
```
...
```{r}
z = 30 #do not use equal sign
q <-42
```

Once an object has been created, you can do things with them.  is like creating a variable on python
```{r}
treatment <- 36
control <- 38
```

Here we make a new object `my_experiment` that is the sum of the treatment and control. Notice that I use `_` and not spaces. Using TAB to type in the rest of the varible name, prevent spelling error, the TAB key 
```{r}
...
notes: try to give object specific name that avoid it havin the same name as other function
...
3. Use the function `mean()` to calculate the mean top speed for the cars. c is concatenated, so concatenated when there are more than 2 objects??
```{r}
...
the above means even you have 100000 values but have 1 NA, the result will be NA

3. How do you interpret this result? What does this mean about NAs?  
It is saying NA is the mean for the following number, it mean it actually disrupt the calculation process and made it the new mean.

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm=T) #na.rm removes the NA values in the vector, and then calculate the mean rm is remove, T is true
```
