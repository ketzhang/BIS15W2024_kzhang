...
years should not be column name
you need x and y axis
if x is year, we dont have it now because year is spread out across the whole data frame, so you need to get it into one column
```{r}
...
expenditures %>% 
  pivot_longer(-expenditure, #not moving expenditure
               names_to = "year", #make a new variable year
               values_to = "bn_dollars") #move the value into billion dollar
```
...
Restrict the data to private education. Sort in ascending order.
```{r}
expenditures %>% 
  pivot_longer(-expenditure,
               names_to = "year",
...
make it to data that you can analysis
```{r}
beachbugs_wide %>% 
  pivot_longer(-year,
               names_to = "site",
