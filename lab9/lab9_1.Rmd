...
the drug is observation
you made colname the observation
```{r}
...
```{r}
income <- read_csv("data/relig_income.csv" )
income
```
...
billboard2 <- 
  billboard %>% 
  pivot_longer(wk1:wk76, # a range of columns, if data is in sequential way
               names_to = "week",
               values_to = "rank", 
...
```{r}
plant <- read_csv("data/plant_data.csv")
```

2. Why are these data not tidy?  
```{r}
#there are too many days listed
```
