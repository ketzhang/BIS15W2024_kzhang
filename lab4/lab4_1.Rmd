...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #readr:: means explict about the package we are using
```
...
```{r}
select(fish, matches("a.+er")) #handy bit of code for the project, look at variable of interest , this means start with letter a and end with er
```
...
Load the data.  whenever you import a data, it is already a data frame, do not use data frame
```{r}
...
```{r}
new_mammals <- select(mammals_new,"genus", "species", "mass")
new_mammals
```
...
```
```{r}
library(janitor) #change all the wrpng spelling names of variables
names(mammals)
```
