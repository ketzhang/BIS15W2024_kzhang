...
dplyr used to transform data
tidyverse is a huge library
select and filter are comments
2. Describe how `select ()` can be used to transform data frames.  
select the variables for building new data frames
select go vertical
3. Describe how `filter ()` can be used to transform data frames. 
filter the variables that you dont want, filter out specific values
filter go horizontal
...
#or you can do this
#glimpse(fish), check the class of variables first
```

4. Calculate the mean length of fish from lake "BO".  
```{r}
new_fish <- filter(fish, lakeid == "BO")
mean(new_fish$length, na.rm = T)
#you can do this too, same as above
#fish_bo <-filter(fish, lakeid == "BO")
#mean(fish_bo$length, na.rm=T)
```
