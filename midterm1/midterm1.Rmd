...
**First determine whether there is NA in the pack data or not, it returned false -> no NA in "pack"**
```{r}
anyNA(wolves$packsize_aug)
```

**Find the maximum pack: druid**
```{r}
wolves %>% 
  group_by(pack) %>% 
  summarize(max_pack = max(packsize_aug, na.rm = T)) %>% 
...
wolves %>% 
  group_by(park) %>% 
  summarize(high_human = max(mort_all)) %>% 
...
**The average pack size of this pack is (year / average pack size)**
...
**I think in 2010, the pack size is because the wolve pack might have migrated, there are zero mortalities recorded and reproduction is also zero.  There are also possibility that the wolves are all dead due to fights and aggressive interactions with other wolves packs.**
```{r}
ynp %>% 
  filter(pack == "druid") %>% 
  filter(biolyr == 2010) %>% 
