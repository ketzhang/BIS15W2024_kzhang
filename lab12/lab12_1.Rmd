...
grouping by a diff categorical variable on x
```{r}
homerange %>% 
...
ON THE TEST
 we are interested how sample change over time
```{r}
deserts %>% 
  filter(species_id == "DM" | species_id == "DS") %>% 
  mutate(year = as.factor(year)) %>%  #you want it to be factor
  group_by(year, species_id) %>% 
  summarize(n=n()) %>%  #look at number of sample
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=2)+ #you can experiment with shapes size you want to
...
homerange %>% 
  ...
  geom_density(color="black", fill="violet")+ 
  labs(title = "Distribution of Body Mass")
```
...
change continuous variables to categorical variation
```{r}
homerange %>% 
  select(log10.mass) %>% 
...
maybe on the test: making category and build plots on them
```{r}
homerange %>% 
