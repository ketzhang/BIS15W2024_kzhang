...
What are the dimensions of the `hbirds` data frame? The `dim()` and `str()` commands provide this information.   3 3 means 3 observation and 3 variable
```{r}
dim(hbirds)
```
...
2. Change the column titled `name` to `scientist`, leave the other column names the same. Print out the data frame names.  basically rebuilding the data frame
```{r}
hsprings <- data_frame(temp_C,scientist = name, spring)
hsprings
```

3. Our scientists forgot to record the depth data for each spring. Here it is as a vector, add it as a new column called depth_ft: `c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)`. Print out the data frame.   overwrite the hspring again
```{r}
